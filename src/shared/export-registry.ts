...
const HISTORY_ENTRY_SCHEMA = Object.freeze({
  ...
  properties: { id: { type: 'string' }, appId: { type: 'string' }, displayName: { type: 'string' }, kind: { enum: ['install', 'build', 'uninstall', 'launch', 'update', 'settings'] }, ok: { type: 'boolean' }, message: { type: 'string' }, messageYue: { type: 'string' }, occurredAt: { type: 'string', format: 'date-time' } },
});
