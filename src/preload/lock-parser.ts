...
export function parseLockBulkMutationResult(value: unknown): LockBulkMutationResult { const base = parseLockMutationResult(value); const input = object(value, 'The bulk lock mutation response was invalid.'); if (!Number.isInteger(input.affectedCount) || Number(input.affectedCount) < 0 || Number(input.affectedCount) > 64 || !Number.isInteger(input.skippedCount) || Number(input.skippedCount) < 0 || Number(input.skippedCount) > 64 || !Array.isArray(input.skippedTargets) || input.skippedTargets.length > 64 || Number(input.skippedCount) !== input.skippedTargets.length || Number(input.affectedCount) + Number(input.skippedCount) > 64 || (input.ok === true && Number(input.affectedCount) < 1) || (input.ok === false && Number(input.affectedCount) > 0)) throw new Error('The bulk lock mutation response was invalid.'); const skippedTargets = input.skippedTargets.map(parseLockTarget); if (new Set(skippedTargets.map((targetValue) => `${targetValue.targetKind}:${targetValue.targetId}`)).size !== skippedTargets.length) throw new Error('The bulk lock mutation response was invalid.'); return Object.freeze({ ...base, affectedCount: Number(input.affectedCount), skippedCount: Number(input.skippedCount), skippedTargets }); }
