...
export interface ExecutableInstallAdapter extends AdapterBase {
  ...
  readonly launchExecutableRelativePaths?: readonly string[];
...
const msi = (
  ...
  checksumAssetPattern, installArguments: ['/qn', '/norestart'], registryDisplayNames, launchExecutableNames, evidence,
});
...
export const INSTALL_ADAPTERS: Readonly<Record<CatalogAppId, InstallAdapter>> = {
  ...
  'material-designer': squirrel('material-designer-squirrel', 'material-designer', /^material-designer-[0-9A-Za-z.+-]+-win-x64-setup\.exe$/, ['Material Designer', 'Open Design'], ['.github/workflows/release.yml: Squirrel.Windows build and installed smoke', 'design/tools/pack/src/win/identity.ts: installed Material Designer.exe identity'], /^material-designer-[0-9A-Za-z.+-]+-win-x64-setup\.exe\.sha256$/, ['Material Designer.exe']),
  'material-bluemap': squirrel('material-bluemap-squirrel', 'material-bluemap', /^Worldlens-[0-9A-Za-z.+-]+-Setup\.exe$/, ['Worldlens', 'Material BlueMap'], ['design/packages/app/electron-builder.config.cjs: Squirrel target and Worldlens.exe identity'], undefined, ['Worldlens.exe']),
  'desktop-material': squirrel('desktop-material-squirrel', 'desktop-material', /^GitHubDesktopSetup-x64\.exe$/, ['GitHub Desktop', 'Desktop Material'], ['app/package.json and build-installers workflow: Squirrel Windows release', 'script/dist-info.ts: GitHubDesktop.exe identifier'], undefined, ['GitHubDesktop.exe']),
  ...
  'material-email': nsis('material-email-nsis', 'material-email', /^Material-Email-[0-9A-Za-z.+-]+-Windows-x64\.exe$/, ['Material Email'], ['Uninstall Material Email.exe'], ['package.json: electron-builder NSIS x64 target', 'scripts/verify-windows-installer.mjs: installed Material Email.exe launch proof'], undefined, ['Material Email.exe']),
  ...
  'qbittorrent-material': squirrel('qbittorrent-material-squirrel', 'qbittorrent-material', /^qBittorrent-Material-[0-9A-Za-z.+-]+-windows-x64-Setup\.exe$/, ['qBittorrent Material'], ['scripts/test-squirrel-package.ps1: versioned qbittorrent.exe launch path'], undefined, ['qbittorrent.exe']),
  'material-winscp': squirrel('material-winscp-squirrel', 'material-winscp', /^WinSCP\.Material\.[0-9A-Za-z.+-]+\.Setup\.exe$/, ['WinSCP Material'], ['forge.config.js: WinSCPMaterial.exe identity', 'docs/packaging-and-updates/installer.md: versioned launch path'], undefined, ['WinSCPMaterial.exe']),
  ...
  'material-office': nsis('material-office-nsis', 'material-office', /^Material-Office-[0-9A-Za-z.+-]+-x64-Setup\.exe$/, ['Material Office'], ['Uninstall Material Office.exe'], ['.github/workflows/build-release.yml: installed Material Office.exe launch proof'], undefined, ['Material Office.exe']),
  ...
  'libreoffice-material': { ...msi('libreoffice-material-msi', 'libreoffice-material', /^LibreOfficeMaterial-Windows-x64\.msi$/, ['LibreOffice Material'], ['windows-msi-manifest.json: program/soffice.exe payload'], /^LibreOfficeMaterial-Windows-x64\.msi\.sha256$/), launchExecutableRelativePaths: ['program/soffice.exe'] },
  'thunderbird-desktop': { id: 'thunderbird-desktop-mozilla-nsis', appId: 'thunderbird-desktop', supported: true, family: 'mozilla-nsis', packageType: 'nsis', assetPattern: /^thunderbird-[0-9A-Za-z.+-]+\.en-US\.win64\.installer\.exe$/, installArguments: ['-ms'], registryDisplayNames: ['Mozilla Thunderbird', 'Thunderbird', 'Material Mail'], launchExecutableNames: ['thunderbird.exe'], uninstallExecutableNames: ['helper.exe'], uninstallArguments: ['/S'], evidence: ['mail/installer/windows/nsis/defines.nsi.in: thunderbird.exe identity'] },
  'bambu-studio': nsis('bambu-studio-nsis', 'bambu-studio', /^BambuStudioMD3-Setup\.exe$/, ['Bambu Studio MD3', 'Bambu Studio'], ['Uninstall.exe'], ['packaging/windows/BambuStudioMD3.nsi: installed bambu-studio.exe identity'], /^BambuStudioMD3-Setup\.exe\.sha256$/, ['bambu-studio.exe']),
  keepassxc: msi('keepassxc-msi', 'keepassxc', /^KeePassXC-[0-9A-Za-z.+-]+-snapshot-x64\.msi$/, ['KeePassXC'], ['CMakeLists.txt: direct-root KeePassXC.exe MSI payload'], undefined, ['KeePassXC.exe']),
  ...
  'material-minecraft-map-editor': {
    ...squirrel('material-minecraft-map-editor-squirrel', 'material-minecraft-map-editor', /^Setup\.exe$/, ['Material Minecraft Map Editor', 'Amulet Map Editor', 'Amulet'], ['release 0.11.0-dev.26 full package: Material Minecraft Map Editor.exe', 'legacy installer/build-squirrel.ps1: Amulet.exe validation'], undefined, ['Material Minecraft Map Editor.exe', 'Amulet.exe']),
  ...
  'material-ollama': inno('material-ollama-inno', 'material-ollama', /^OllamaSetup\.exe$/, ['Ollama'], ['ollama app.exe'], ['release v0.0.0-build.17: OllamaSetup.exe', 'app/CMakeLists.txt: bundled Ollama Windows payload'], undefined),
  ...
  'material-virtualbox': nsis('material-virtualbox-nsis', 'material-virtualbox', /^VirtualBox-[0-9A-Za-z.+-]+-Setup\.exe$/, ['Oracle VM VirtualBox', 'VirtualBox'], ['uninstall.exe'], ['release v7.2.97-ci.109: reviewed unsigned VirtualBox-7.2.97-Setup.exe', 'doc/installer/WindowsHostInstallerNSIS.md: exact /S install and uninstall contract', 'build.bat and build-installer.bat: supported root build routes'], undefined, ['VirtualBox.exe']),
  'material-winforge': squirrel('material-winforge-squirrel', 'material-winforge', /^WinForge-Material-3-Preview-Setup-[0-9A-Za-z.+-]+\.exe$/, ['WinForge Material 3 Preview', 'WinForge'], ['release v1.0.95: material-winforge-1.0.95-full.nupkg contains lib/net45/WinForge Material 3 Preview.exe', 'build.bat and build-installer.bat: supported root build routes'], undefined, ['WinForge Material 3 Preview.exe']),
  'material-winutil': squirrel('material-winutil-squirrel', 'material-winutil', /^MaterialSystemUtility-Setup\.exe$/, ['Material System Utility', 'Material WinUtil'], ['release v0.1.16501: MaterialSystemUtility-0.1.16501-full.nupkg contains lib/net45/Material System Utility.exe', 'build.bat and build-installer.bat: supported root build routes'], undefined, ['Material System Utility.exe']),
...
/** Launch support is explicit adapter data: portable paths or reviewed executable basenames only. */
export function adapterSupportsLaunch(appId: string): boolean {
  const adapter = adapterFor(appId);
  return adapter.supported && (adapter.family === 'portable-zip'
    || (adapter.launchExecutableNames?.length ?? 0) > 0
    || (adapter.launchExecutableRelativePaths?.length ?? 0) > 0);
}
...
export function validateAdapterCoverage(): void {
  ...
  for (const appId of CATALOG_APP_IDS) {
    ...
    if (adapter.supported && adapter.family !== 'portable-zip') {
      const launchNames = adapter.launchExecutableNames ?? [];
      if (new Set(launchNames.map((name) => name.toLocaleLowerCase())).size !== launchNames.length
        || launchNames.some((name) => path.basename(name) !== name || !/^[^\\/:*?"<>|\r\n]{1,128}\.exe$/i.test(name))) {
        throw new Error(`Adapter ${adapter.id} has an invalid or duplicate launch executable identity.`);
      }
      const launchPaths = adapter.launchExecutableRelativePaths ?? [];
      const normalizedPaths = launchPaths.map((relative) => relative.replaceAll('\\', '/'));
      if (new Set(normalizedPaths.map((relative) => relative.toLocaleLowerCase())).size !== normalizedPaths.length
        || normalizedPaths.some((relative) => path.posix.isAbsolute(relative)
          || path.posix.normalize(relative) !== relative
          || relative.startsWith('../')
          || !/^(?:[^/:*?"<>|\r\n]+\/)*[^/:*?"<>|\r\n]{1,128}\.exe$/i.test(relative))) {
        throw new Error(`Adapter ${adapter.id} has an invalid or duplicate launch executable relative path.`);
      }
    }
  }
}
