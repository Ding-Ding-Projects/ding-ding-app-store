...
export const INSTALL_ADAPTERS: Readonly<Record<CatalogAppId, InstallAdapter>> = {
  ...
  'libreoffice-material': msi('libreoffice-material-msi', 'libreoffice-material', /^LibreOfficeMaterial-Windows-x64\.msi$/, ['LibreOffice Material'], ['.github/workflows/windows-installer.yml: CPack MSI release with administrative extraction proof'], /^LibreOfficeMaterial-Windows-x64\.msi\.sha256$/),
  'thunderbird-desktop': { id: 'thunderbird-desktop-mozilla-nsis', appId: 'thunderbird-desktop', supported: true, family: 'mozilla-nsis', packageType: 'nsis', assetPattern: /^thunderbird-[0-9A-Za-z.+-]+\.en-US\.win64\.installer\.exe$/, installArguments: ['-ms'], registryDisplayNames: ['Mozilla Thunderbird', 'Thunderbird', 'Material Mail'], uninstallExecutableNames: ['helper.exe'], uninstallArguments: ['/S'], evidence: ['.github/workflows/windows-installer.yml: Mozilla NSIS package built by mach package', 'mail/installer/windows/nsis/installer.nsi and uninstaller.nsi: setup.exe and helper.exe lifecycle'] },
  'bambu-studio': squirrel('bambu-studio-squirrel', 'bambu-studio', /^Setup\.exe$/, ['Bambu Studio MD3', 'Bambu Studio'], ['build-installer.bat: unsigned Squirrel.Windows packaging route', 'scripts/ci/Test-SquirrelWindowsPackage.ps1: Setup.exe, RELEASES, full nupkg, checksum, and unsigned-package validation'], /^Setup\.exe\.sha256$/, ['bambu-studio.exe']),
