...
export const INSTALL_ADAPTER_IDS = [
  ...
  'minecraft-world-downloader-squirrel', 'codex-material-msi', 'libreoffice-material-msi',
...
export const INSTALL_ADAPTERS: Readonly<Record<CatalogAppId, InstallAdapter>> = {
  ...
  'material-office': nsis('material-office-nsis', 'material-office', /^Material-Office-[0-9A-Za-z.+-]+-x64-Setup\.exe$/, ['Material Office'], ['Uninstall Material Office.exe'], ['package.json: electron-builder NSIS x64 target', 'docs/release/windows-installer.md: silent install/uninstall lifecycle'], /^Material-Office-[0-9A-Za-z.+-]+-x64-Setup\.exe\.sha256$/),
  'minecraft-world-downloader': squirrel('minecraft-world-downloader-squirrel', 'minecraft-world-downloader', /^World\.Downloader\.Studio-Setup-[0-9A-Za-z.+-]+\.exe$/, ['World Downloader Studio'], ['app/electron-builder.yml: Squirrel x64 target and versioned setup artifact', '.github/workflows/release.yml: setup executable, RELEASES index, and full package publication']),
