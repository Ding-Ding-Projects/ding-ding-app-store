...
void app.whenReady().then(async () => {
  ...
  ipcMain.handle('source-jobs:outputs', (event, jobId: unknown) => event.sender === mainWindow?.webContents && typeof jobId === 'string' ? sourceJobs.outputs(jobId) : Promise.reject(new Error('Blocked source output request from an unknown renderer.')));
  ipcMain.handle('source-jobs:export-output', async (event, request: SourceOutputExportRequest) => {
    if (event.sender !== mainWindow?.webContents) return Promise.reject(new Error('Blocked source output export request from an unknown renderer.'));
    const format = request?.format === 'zip' ? 'zip' : 'json';
    const suggestedName = `source-output-${request?.jobId ?? 'job'}.${format}`;
    const picked = await dialog.showSaveDialog(mainWindow!, { title: 'Export source output', defaultPath: suggestedName, filters: [{ name: format === 'zip' ? 'ZIP archive' : 'JSON manifest', extensions: [format] }] });
    if (picked.canceled || !picked.filePath) return { ok: false, format, suggestedName, message: 'Source output export was cancelled; no file was created.' };
    try { return await sourceJobs.exportOutput(request, picked.filePath); }
    catch { return { ok: false, format, suggestedName, message: 'The source output could not be exported; no success was claimed.' }; }
  });
  ...
  ipcMain.handle('authenticator:create-group', (event, request: unknown) => event.sender === mainWindow?.webContents ? authenticatorAllowed().then((allowed) => allowed ? authenticator.createGroup(request as AuthenticatorGroupCreateRequest) : authenticatorRestrictedMutation()) : Promise.reject(new Error('Blocked authenticator group request from an unknown renderer.')));
  ipcMain.handle('authenticator:rename-group', (event, request: unknown) => event.sender === mainWindow?.webContents ? authenticatorAllowed().then((allowed) => allowed ? authenticator.renameGroup(request as AuthenticatorGroupRenameRequest) : authenticatorRestrictedMutation()) : Promise.reject(new Error('Blocked authenticator group request from an unknown renderer.')));
  ipcMain.handle('authenticator:reorder-group', (event, request: unknown) => event.sender === mainWindow?.webContents ? authenticatorAllowed().then((allowed) => allowed ? authenticator.reorderGroup(request as AuthenticatorGroupReorderRequest) : authenticatorRestrictedMutation()) : Promise.reject(new Error('Blocked authenticator group request from an unknown renderer.')));
  ipcMain.handle('authenticator:collapse-group', (event, request: unknown) => event.sender === mainWindow?.webContents ? authenticatorAllowed().then((allowed) => allowed ? authenticator.collapseGroup(request as AuthenticatorGroupCollapseRequest) : authenticatorRestrictedMutation()) : Promise.reject(new Error('Blocked authenticator group request from an unknown renderer.')));
  ipcMain.handle('authenticator:delete-group', (event, request: unknown) => event.sender === mainWindow?.webContents ? authenticatorAllowed().then((allowed) => allowed ? authenticator.deleteGroup(request as AuthenticatorGroupDeleteRequest) : authenticatorRestrictedMutation()) : Promise.reject(new Error('Blocked authenticator group request from an unknown renderer.')));
  ipcMain.handle('authenticator:move-to-group', (event, request: unknown) => event.sender === mainWindow?.webContents ? authenticatorAllowed().then((allowed) => allowed ? authenticator.moveToGroup(request as AuthenticatorGroupBulkMoveRequest) : { ok: false, movedIds: [], skippedIds: [], message: 'Authenticator groups are unavailable.', messageYue: 'Authenticator 分組暫時用唔到。' }) : Promise.reject(new Error('Blocked authenticator group request from an unknown renderer.')));
  ...
  ipcMain.handle('support:bulk-advance', (event, request: SupportTicketBulkAdvanceRequest) => event.sender === mainWindow?.webContents ? lockSupport.bulkAdvanceTickets(request) : Promise.reject(new Error('Blocked Support Tickets request from an unknown renderer.')));
  ...
  ipcMain.handle('history:archive-7z', async (event, request: unknown) => {
    if (event.sender !== mainWindow?.webContents) return Promise.reject(new Error('Blocked history 7z request from an unknown renderer.'));
    const token = await historyToken();
    if (token === null) return Promise.reject(new Error('Protected local history is locked or unavailable.'));
    const result = await history.archive7z(request as History7zRequest);
    if (!historyAccess.isSessionCurrent(token)) throw new Error('Protected local history became unavailable before the 7z result was returned.');
    return result;
  });
