...
function inline(text: string, onOpen: (id: string) => void, knownIds: ReadonlySet<string>): ReactNode[] {
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (token.startsWith('[')) {
      ...
      pieces.push(id && knownIds.has(id)
        ? <button className="article-link" key={`link-${index}`} onClick={() => onOpen(id)}>{link?.[1]}</button>
        : <span key={`link-${index}`} title={id ? 'Unavailable documentation article link' : undefined}>{link?.[1] ?? token}</span>);
...
export function MarkdownArticle({ article, onOpen, knownIds }: { article: GeneratedDocArticle; onOpen(id: string): void; knownIds: ReadonlySet<string> }) {
  ...
  const flushParagraph = () => {
    ...
    output.push(<p key={`p-${key++}`}>{inline(paragraph.join(' '), onOpen, knownIds)}</p>);
    paragraph = [];
  };
  const flushBullets = () => {
    if (!bullets.length) return;
    output.push(<ul key={`ul-${key++}`}>{bullets.map((item, itemIndex) => <li key={itemIndex}>{inline(item, onOpen, knownIds)}</li>)}</ul>);
    bullets = [];
  };
