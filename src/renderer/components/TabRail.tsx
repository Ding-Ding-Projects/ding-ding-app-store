...
export function TabContextMenu({ target, workspace, settings, locks, dispatch, onClose, onRename, onMovePicker, onManageLock, announce }: {
  target: MenuTarget; workspace: TabWorkspace; settings: UserSettings;
  locks?: LocksApi;
  dispatch(action: WorkspaceAction): void; onClose(): void; onRename(groupId: string): void; onMovePicker(tabId: TabId): void; onManageLock?(target: LockTarget): void; announce(message: string): void;
}) {
...
export function TabRail({ settings, workspace, dispatch, locks, updatesBadge, onOpenPalette, announce, openOverflow, onOverflowHandled, openTabRegex, onTabRegexHandled, renameGroupId: renameRequest, onRenameHandled, onManageLock }: TabRailProps) {
  ...
  const activate = useCallback((id: TabId) => {
    ...
    if (groupTarget && locks.isLocked(groupTarget)) {
    ...
    if (locks.isLocked({ targetKind: 'tab', targetId: id })) {
      setMenu({ kind: 'tab', id });
    ...
    setFocusKey(`t:${id}`);
  }, [announce, dispatch, locks, workspace.tabs]);
  ...
  const onRowKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>, index: number, row: TabRow) => {
    ...
    if (key === 'Enter' || key === ' ') {
      ...
      else if (locks.isLocked({ targetKind: 'group', targetId: row.group.id })) {
    ...
    if (key === 'ArrowRight') {
      ...
      if (row.kind === 'header' && row.group.collapsed) {
        if (locks.isLocked({ targetKind: 'group', targetId: row.group.id })) {
          setMenu({ kind: 'group', groupId: row.group.id });
  ...
  const renderTab = (row: TabRowOnly, index: number) => {
    ...
    return (
      <TabRailItem
        ...
        locked={locks.isLocked({ targetKind: 'tab', targetId: row.tab.id })}
      ...
      blocks.push(
        <div className="tab-region" key={row.key}>
          <TabGroupHeader
            ...
            locked={locks.isLocked({ targetKind: 'group', targetId: row.group.id })}
            bodyId={bodyId}
  ...
  return (
    <nav className="navigation" aria-label={label(settings, 'Tabs', '分頁')} {...el('nav-rail')}>
      ...
      {menu && (
        <TabContextMenu target={menu} workspace={workspace} settings={settings} locks={locks} dispatch={dispatch} announce={announce} onRename={(groupId) => setRenamingGroupId(groupId)} onMovePicker={(tabId) => setMovePickerTabId(tabId)} onManageLock={onManageLock} onClose={() => setMenu(null)} />
      )}
