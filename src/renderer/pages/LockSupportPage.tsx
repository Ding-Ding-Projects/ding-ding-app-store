...
export function LockSupportPage({ settings, workspace, locks, support, notify, matcher, initialTarget }: {
  ...
  const completeBulkRemove = async () => { const result = await locks.bulkRemove({ confirmed: true, items: selectedVisibleRecords.map(({ targetKind, targetId }) => ({ targetKind, targetId, credential: bulkCredentials[targetKey({ targetKind, targetId })] ?? '' })) }); if (!result.ok) throw new Error(result.message); setBulkRemoveOpen(false); clearVisible(); };
  useEffect(() => { if (selectionAnchor !== null && selectionAnchor >= visibleRecords.length) setSelectionAnchor(visibleRecords.length ? visibleRecords.length - 1 : null); }, [selectionAnchor, visibleRecords.length]);
  useEffect(() => {
    if (selectedRecord) {
  ...
  return (
    <div className="lock-support-grid">
      ...
      {bulkRemoveOpen && <DestructiveConfirmDialog settings={settings} title={label(settings, `Remove ${selectedVisibleRecords.length} selected locks?`, `移除${selectedVisibleRecords.length} 個所選鎖？`)} description={label(settings, 'Preview: each selected lock is checked with its own credential. Missing, filtered-out, or mismatched locks are skipped and reported by exact count; no other lock is touched.', '預覽：每個所選鎖都會用自己嘅憑證檢查。遺失、篩走或者唔匹配嘅鎖會跳過並報告實際數目；其他鎖唔會郁。')} actionLabel={label(settings, `REMOVE ${selectedVisibleRecords.length} LOCKS`, `移除${selectedVisibleRecords.length} 個鎖`)} credentialFields={selectedVisibleRecords.map((record) => ({ id: `bulk-lock-credential-${targetKey(record).replace(/[^A-Za-z0-9_-]/g, '-')}`, label: `${targetLabel(record)} — ${record.credentialKind === 'totp' ? 'Current TOTP code' : 'Current password'}`, value: bulkCredentials[targetKey(record)] ?? '', onChange: (value: string) => setBulkCredentials((current) => ({ ...current, [targetKey(record)]: value })) }))} onClose={() => setBulkRemoveOpen(false)} onConfirm={completeBulkRemove} />}
