...
export function AppCard({ app, installedRecord, settings, onAction, onManagedUpdate, onCancelInstall, managedUpdate, operationProgress, searchLabel, runningAction, selected, onSelect }: { app: CatalogApp; installedRecord: InstalledAppRecord | undefined; settings: UserSettings; onAction: (kind: ActionKind, app: CatalogApp, trigger: HTMLButtonElement) => void; onManagedUpdate: (kind: 'download' | 'cancel' | 'restart', app: CatalogApp, trigger: HTMLButtonElement) => void; onCancelInstall: (app: CatalogApp, trigger: HTMLButtonElement) => void; managedUpdate: ManagedUpdateState | undefined; operationProgress: OperationProgressEvent | undefined; searchLabel: ReactNode; runningAction: RunningAction | null; selected: boolean; onSelect(checked: boolean, shiftKey: boolean): void }) {
  ...
  const proofReason = app.proofStatus === 'blocked-until-proof'
    ? label(settings, `Unavailable until clean-Windows lifecycle proof is verified${app.proofTargetId ? ` (${app.proofTargetId})` : ''}. Install, build, and update actions remain blocked.`, `未能使用，要等 clean-Windows 生命週期證據驗證${app.proofTargetId ? `（${app.proofTargetId}）` : ''}。安裝、建置同更新操作都會封鎖。`)
    : label(settings, 'Unavailable because this catalog row has no verified proof status. Install, build, and update actions remain blocked.', '未能使用，因為呢個目錄項目冇已驗證嘅證據狀態。安裝、建置同更新操作都會封鎖。');
    ...
    <article className="app-card" data-app-id={app.id} data-availability={app.availability} {...el('app-card')}>
      ...
      <div className="card-heading"><h3 {...el('app-card-title')}>{searchLabel}</h3><span className={`status-pill ${proofBlocked ? 'blocked' : app.updateState}`} {...el('status-pill')}>{proofBlocked ? label(settings, 'Blocked until proof', '證據未齊，暫停') : appStatusLabel(settings, app.updateState)}</span></div>
        ...
        <div className="card-actions">
          {!proofBlocked && app.availability === 'installable' && !discoveryOnly && <button className="filled-button" data-install-action={app.id} {...el('button-filled')} disabled={operationBusy} aria-busy={installBusy} title={busyExplanation} onClick={(event) => onAction('install', app, event.currentTarget)}><Icon>download</Icon>{installBusy ? label(settings, 'Installing…', '安裝緊…') : label(settings, managed ? 'Reinstall' : 'Install', managed ? '重新安裝' : '安裝')}</button>}
