    ...
    <article className="app-card" data-app-id={app.id} data-availability={app.availability} {...el('app-card')}>
      ...
      <div className="card-heading"><h3 {...el('app-card-title')}>{searchLabel}</h3><span className={`status-pill ${proofBlocked ? 'blocked' : app.updateState}`} {...el('status-pill')}>{proofBlocked ? label(settings, 'Blocked until proof', '證據未齊，暫停') : appStatusLabel(settings, app.updateState)}</span></div>
        ...
        <div className="card-actions">
          {!proofBlocked && managed && app.launchAvailable && <button className="filled-button" data-launch-action={app.id} {...el('button-filled')} disabled={operationBusy} aria-busy={launchBusy} title={busyExplanation ?? label(settings, 'Start the exact reviewed executable from this App Store-managed installation.', '由呢個 App Store 管理嘅安裝位置啟動已審核嘅指定程式。')} onClick={(event) => onAction('launch', app, event.currentTarget)}><Icon>open_in_new</Icon>{launchBusy ? label(settings, 'Launching…', '啟動緊…') : label(settings, 'Launch', '啟動')}</button>}
          {!proofBlocked && managed && !app.launchAvailable && <button className="tonal-button" disabled title={label(settings, 'Launch unavailable: this adapter has no reviewed installed executable identity.', '未能啟動：呢個適配器未有已審核嘅安裝程式身份。')}><Icon>block</Icon>{label(settings, 'Launch unavailable', '未能啟動')}</button>}
          {!proofBlocked && app.availability === 'installable' && !discoveryOnly && <button className={managed ? 'tonal-button' : 'filled-button'} data-install-action={app.id} {...el(managed ? 'button-tonal' : 'button-filled')} disabled={operationBusy} aria-busy={installBusy} title={busyExplanation} onClick={(event) => onAction('install', app, event.currentTarget)}><Icon>download</Icon>{installBusy ? label(settings, 'Installing…', '安裝緊…') : label(settings, managed ? 'Reinstall' : 'Install', managed ? '重新安裝' : '安裝')}</button>}
          ...
          {!proofBlocked && managed && app.updateState === 'available' && app.installedVersion && managedUpdate?.status !== 'ready' && (
            <button className="tonal-button" disabled={operationBusy} aria-busy={managedUpdate?.status === 'downloading'} title={managedUpdate?.status === 'offline' ? managedUpdate.message : 'Download and verify this stable release. Installation starts only after you choose Install update.'} onClick={(event) => onManagedUpdate('download', app, event.currentTarget)}>
          ...
          {!proofBlocked && managed && managedUpdate?.status === 'downloading' && managedUpdate.appId === app.id && <button className="text-button" onClick={(event) => onManagedUpdate('cancel', app, event.currentTarget)}>{label(settings, 'Cancel', '取消')}</button>}
          {!proofBlocked && managed && managedUpdate?.status === 'ready' && managedUpdate.appId === app.id && <button className="filled-button" disabled={operationBusy} onClick={(event) => onManagedUpdate('install', app, event.currentTarget)} title={label(settings, 'The verified installer is staged. Close the target application, then choose this explicit action to install it; no discovery path launches an installer.', '已驗證安裝程式已暫存。先關閉目標應用程式，再揀呢個明確操作安裝；偵測路徑永遠唔會啟動安裝程式。')}><Icon>system_update_alt</Icon>{label(settings, 'Install update', '安裝更新')}</button>}
