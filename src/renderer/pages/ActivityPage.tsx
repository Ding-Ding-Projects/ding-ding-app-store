    ...
    <section className="history-panel">
      ...
      <div className="card-actions">
        ...
        {exportFormat === '7z' && <div className="archive-7z-options" aria-label={label(settings, '7z archive options', '7z 壓縮檔選項')}>
          ...
          <label>{label(settings, 'Split volume bytes', '分卷 bytes')}<input type="number" min={1048576} max={16777216} step={1048576} value={archive7zOptions.splitBytes ?? ''} placeholder={label(settings, 'No split', '唔分卷')} onChange={(event) => setArchive7zOptions((current) => ({ ...current, splitBytes: event.target.value === '' ? null : Number(event.target.value) }))} /></label>
          <label title={label(settings, 'Unavailable until a secure password flow and approved encoder exist.', '未有安全密碼流程同已審核 encoder 前未能使用。')}><input type="checkbox" disabled checked={false} readOnly />{label(settings, 'AES-256 content encryption (unavailable)', 'AES-256 內容加密（未能使用）')}</label>
          <label title={label(settings, 'Unavailable until a secure password flow and approved encoder exist.', '未有安全密碼流程同已審核 encoder 前未能使用。')}><input type="checkbox" disabled checked={false} readOnly />{label(settings, 'Encrypt headers (unavailable)', '加密標頭（未能使用）')}</label>
          <p className="supporting">{label(settings, '7z is unavailable in this build until an approved in-process encoder and secure credential flow are shipped; ZIP remains the working round-trip archive.', '呢個版本未有已審核嘅內置 7z encoder 同安全密碼流程，所以 7z 暫時未能使用；ZIP 仍然係可重用嘅壓縮檔。')}</p>
        </div>}
