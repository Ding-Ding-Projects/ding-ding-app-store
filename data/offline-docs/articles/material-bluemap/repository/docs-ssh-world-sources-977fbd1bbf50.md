...
This is the read side. [Rendering on a remote host](app-doc://article/material-bluemap.repository.3c1ea19ffb79c9bd) is the other
...
Exactly the same rule [remote rendering](app-doc://article/material-bluemap.repository.3c1ea19ffb79c9bd#the-host-key-is-a-decision-not-a-default)
...
- **No password, ever.** The SSH options this reuses make the client refuse one even when a
  host offers it — see [remote rendering](app-doc://article/material-bluemap.repository.3c1ea19ffb79c9bd#authentication-keys-only-never-a-password)
...
- [Rendering on a remote host](app-doc://article/material-bluemap.repository.3c1ea19ffb79c9bd) — the other direction over the same SSH
  connection, host-key and transfer machinery.
- [Worlds from somebody else's release](app-doc://article/material-bluemap.repository.ae963fc72bd07ef6) — a world from a GitHub release
  instead of a machine you run yourself.
- [Rendering a world in GitHub Actions](app-doc://article/material-bluemap.repository.8be4992e8687fe6b) — where a scheduled render's
  change detection would use the cheap survey this feature exposes.
- [Renders that survive being interrupted](app-doc://article/material-bluemap.repository.334128592026c1b2) — the same "carry on, do
