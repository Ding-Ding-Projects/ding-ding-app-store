...
- [Worlds from somebody else's release](app-doc://article/material-bluemap.repository.ae963fc72bd07ef6) — the other way a world moves between
  machines.
- [Worlds hosted on your own SSH server](app-doc://article/material-bluemap.repository.977fbd1bbf50c95b) — the read direction over this
  same connection, host-key and transfer machinery: a world fetched from a server you own
  rather than sent to one.
- [Rendering a world in GitHub Actions](app-doc://article/material-bluemap.repository.8be4992e8687fe6b) — the other machine that can render
  for you, and the one that needs no server of your own.
- [Renders that survive being interrupted](app-doc://article/material-bluemap.repository.334128592026c1b2) — the same promise for a render
  on this computer, and the incremental behaviour that makes carrying one on cheap.
- [Running the engine on this computer, or in a container](app-doc://article/material-bluemap.repository.5db9c82f867c7897) — where the
