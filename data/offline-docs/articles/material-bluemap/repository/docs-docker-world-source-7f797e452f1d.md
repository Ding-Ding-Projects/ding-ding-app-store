...
**What this is not connected to, and why not:** [Scheduled re-rendering](app-doc://article/material-bluemap.repository.2e16c8f63b4747c4)'s
...
- [Running the engine on this computer, or in a container](app-doc://article/material-bluemap.repository.5db9c82f867c7897) — the Docker
  path a *render* takes, including the mount rules `dockerworld/`'s own host-path checks are
  modelled on.
- [Rendering on a remote host](app-doc://article/material-bluemap.repository.3c1ea19ffb79c9bd) — the SSH primitives (`CommandRunner`,
  `FileTransfer`, `chooseTransfer`) this module reuses rather than reimplementing.
- [Worlds from somebody else's release](app-doc://article/material-bluemap.repository.ae963fc72bd07ef6) — the other input-side world source,
  and the same "fully built, not yet wired to the UI" gap this document names for the same reason.
- [Scheduled re-rendering](app-doc://article/material-bluemap.repository.2e16c8f63b4747c4) — the GitHub Actions lane `dockerWorldFingerprint`
  is deliberately *not* wired into, and why: a GitHub-hosted runner has no route to a local
  Docker daemon.
- [Worlds hosted on your own SSH server](app-doc://article/material-bluemap.repository.977fbd1bbf50c95b) — the other locally-reachable,
...
- [Backing up a world or a rendered map](app-doc://article/material-bluemap.repository.fdf6f35fb97bc1a7) — why this project never reaches for Git
