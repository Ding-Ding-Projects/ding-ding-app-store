...
Home screen and the command palette's own row (Ctrl+Shift+F, then
...
*Image omitted from the offline bundle: The make-a-map wizard's World step, with a real Minecraft world folder typed in, validated, and reporting one dimension and one region file.*

*Image omitted from the offline bundle: The wizard's Review step before consent: the same download-has-not-been-accepted warning and "Open the setting" link described above.*

*Image omitted from the offline bundle: The Mojang download consent settings row, reached from that warning, with consent just accepted.*

*Image omitted from the offline bundle: The render starting: BlueMap's own Java engine (5.22-27, on this run's Java 25.0.3) beginning against the world from the step above.*

*Image omitted from the offline bundle: The render in progress.*

*Image omitted from the offline bundle: The finished map, opened in the viewer directly from the render panel's "Open the map" button.*
...
**Edit appearance…**, Shift+F10 for the same menu from the keyboard, or
Ctrl+Shift+F10 straight to the editor — with the full Word-depth
...
- [Language modes and funny levels](app-doc://article/material-bluemap.repository.bfab28ebad332d8c) — why the consent and licence statements
  live in the EXACT catalogue and are the same text at level 1 and level 5.
- [Tabbed navigation](app-doc://article/material-bluemap.repository.7bfd5537157accc8) — the tab system the viewer reuses, and what its four
  searches and bulk closes do.
- [Appearance editors](app-doc://article/material-bluemap.repository.f1073e3c30c44e85) — the per-element editor, the infinite colour picker
  and the typography controls the viewer exposes.
- [The regex builder and the search bars it reaches](app-doc://article/material-bluemap.repository.6ff379945583bb75) — the engine the licence
  search runs on and the bounds it evaluates under.
- [Command palette](app-doc://article/material-bluemap.repository.e8c7bcf1c71044a6) — the other route to every setting, including a panel's
  placement.
