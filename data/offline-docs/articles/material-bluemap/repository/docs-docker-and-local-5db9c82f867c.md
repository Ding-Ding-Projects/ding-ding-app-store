...
` — and the container is started with `--init` so that the daemon's SIGTERM actually
...
- [Automatic dependency provisioning](app-doc://article/material-bluemap.repository.52abb3b0af63965f) — the Download Java button that
  gets a machine with no JVM ready for the local route this article describes, without needing
  Docker at all.
- [Automatic repair when a render or the web server fails to start](app-doc://article/material-bluemap.repository.dc5d12936768cb23) — what
  happens next when one of these runs does not start.
- [Renders that survive being interrupted](app-doc://article/material-bluemap.repository.334128592026c1b2) — what a cancelled or crashed
  render leaves behind, and how the next one resumes.
- [Rendering on a remote host](app-doc://article/material-bluemap.repository.3c1ea19ffb79c9bd) — the same container problem over SSH, plus a
  world upload that can be interrupted and carried on.
- [Rendering a world in GitHub Actions](app-doc://article/material-bluemap.repository.8be4992e8687fe6b) — the other place the engine runs
