...
starts [`render-world.yml`](app-doc://article/material-bluemap.repository.8be4992e8687fe6b) when it has. A world that has not
...
- **A check** runs whenever `scheduled-render.yml`'s job executes. GitHub's `schedule:`
  ...
  *configured* cadence says a check is actually due yet. Most hourly wake-ups, on a daily or
  ...
  fetched.
- **A render** — dispatching [`render-world.yml`](app-doc://article/material-bluemap.repository.8be4992e8687fe6b), the workflow
...
| `CIRENDER_SCHEDULE_WORLD_SOURCE` | `repository`, `release-asset`, `url` or `git` — the same choices [`render-world.yml`](app-doc://article/material-bluemap.repository.8be4992e8687fe6b) accepts. |
