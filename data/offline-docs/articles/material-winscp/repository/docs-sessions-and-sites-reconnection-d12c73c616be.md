...
- **Closed adapters are retired before retry.** Their listeners are removed and
  the session reference is cleared before the reconnect timer is scheduled, so
  late events from a dead transport cannot appear in the replacement session.
...
- **Unattended reconnection needs a stored credential**, which is a deliberate
  ...
  [credential storage](app-doc://article/material-winscp.repository.9d2325dcc3bf8dd4).
...
- [Tunnels and proxies](app-doc://article/material-winscp.repository.abd9b694c21333cb) — reconnecting a two-hop connection.
- [The queue](app-doc://article/material-winscp.repository.a97eb2e4ae1a0889) — what happens to in-flight transfers.
- [Host key verification](app-doc://article/material-winscp.repository.0748f1d3b696a738) — why a changed key stops everything.
- [Notifications](app-doc://article/material-winscp.repository.e5539f064fd1d9fe) — how reconnection reports itself without a modal.
