...
| `/ini` names a missing or unreadable file | The parent directory remains usable for a first save, and a warning names the path and explains the fallback. | Yes — correct the path |
...
- `test/progparams.test.js` covers the production startup adapter, duplicate
  first-match behaviour, bracketed values, both switch marks, path safety and
  the recoverable `/ini` warning/root decision.
...
- [Custom commands](app-doc://article/material-winscp.repository.0f1843719a666217) — quoting and command construction.
- [The console](app-doc://article/material-winscp.repository.2036717365155926) — scripting and console entry points.
- [Site manager](app-doc://article/material-winscp.repository.3daf06486972e947) — stored sessions and
