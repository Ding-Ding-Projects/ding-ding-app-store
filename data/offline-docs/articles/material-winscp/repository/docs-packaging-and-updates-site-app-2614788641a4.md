...
[the documentation site](app-doc://article/material-winscp.repository.8b4f79acaa2c8578).
...
- Reorder by drag or Ctrl+←/→; pin with
  Ctrl+P. Every context-menu item shows its shortcut.

## Accessibility

- `tablist`/`tab` roles with roving `tabindex`, `aria-selected`, and live
  `aria-controls`; ← → Home End
  Enter Delete all operate the strip.
- Focus is always visible: `:focus-visible` draws a 2 px outline, and no rule
  removes it without replacing it.
- Every popover traps Tab while open, closes on Esc, and
...
- `node --test test/site-app.test.js` — **37 tests, 37 passing.** With
  `site/src/lib/`, `app.js` and `app.css` moved aside, **0 pass and 37 fail**:
...
[the documentation site](app-doc://article/material-winscp.repository.8b4f79acaa2c8578#publishing) — the workflow has still never run,
and Pages is still not enabled on the repository.

## Suggested articles

- [The documentation site](app-doc://article/material-winscp.repository.8b4f79acaa2c8578) — the builder, the base path, and what `--verify` proves.
- Search and regex — the same builder, in the desktop app.
- [Language modes](app-doc://article/material-winscp.repository.728997f194376526) — the dictionary format shared with the app.
- [The dim sum surprise](app-doc://article/material-winscp.repository.e023cb683da2e265) — the catalog the site bundles.
- [Continuous integration](app-doc://article/material-winscp.repository.8db778203c304457) — the other workflow, and the release loop this one avoids.
