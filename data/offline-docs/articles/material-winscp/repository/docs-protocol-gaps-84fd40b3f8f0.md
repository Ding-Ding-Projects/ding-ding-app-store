...
[`docs/porting-mandate.md`](app-doc://article/material-winscp.repository.be3ddf779abd1381) is explicit that this is a port
...
| **Custom SFTP subsystem command** | `sftpServer` | ⬜ Not available | `ssh2`'s public SFTP API always requests the literal `sftp` subsystem. The adapter now refuses a non-empty command before opening SSH instead of silently running the wrong remote program. |
| **Minimum transfer packet size** | `sftpMinPacketSize` | ⬜ Not wired | The setting is retained for session compatibility, but the reachable adapter does not consume it. Transfer chunk sizing is controlled by `sftpMaxPacketSize` and the server/engine ceiling. |
