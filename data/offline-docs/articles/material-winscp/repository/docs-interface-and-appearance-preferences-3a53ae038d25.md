...
The Preferences dialog is generated from `prefpages.js`: 206 declared controls cover 207 persisted paths, including the compatibility companion for double-click behaviour, and every control points at a real path in `defaults.js`. A normal edit goes through the real preload bridge:
...
Suggested related articles: Search and regex,
Accessibility and languages, and
Version history.
