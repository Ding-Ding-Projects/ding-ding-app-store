...
| File | What it covers |
| --- | --- |
| [overview.md](app-doc://article/material-designer.repository.e5a5102f18de6b16) | The daemon / web / desktop / packaged / landing-page split, the process and data-flow between them, ports and bind addresses, the command-line entry point's role, the shared packages, and the exact toolchain versions. **Start here.** |
| [daemon.md](app-doc://article/material-designer.repository.d2a69001b7e2ab97) | The only stateful process: what it owns, its startup order, the three probe endpoints, the command-line entry point, every network and path variable, and why reaching its API is equivalent to shell access. |
| [web-runtime.md](app-doc://article/material-designer.repository.1c2b5224cec132bc) | The single-page interface and the three genuinely different ways it reaches the daemon — development rewrites, static export, and packaged server-side rendering — plus the 19 locales, the boundary it may not cross, and the Material Design 3 token layer as it currently stands. |
| [desktop-shell.md](app-doc://article/material-designer.repository.bdd2e1ad8ecb5375) | The Electron main process: why it never guesses the web port, the sidecar control channel that doubles as the test harness, the frameless Windows window and its renderer-drawn title bar, the window-control channels, and the preload trust boundary. |
| [packaged-runtime.md](app-doc://article/material-designer.repository.224f711bf1d0aba7) | What launches when somebody runs the installed application: the launcher, the `od://` scheme and its retry, sidecar stamps and namespaces, the child-environment allowlist, the namespace-scoped path layout, the four release channels, and a packaged-versus-development table. |
| [data-directory.md](app-doc://article/material-designer.repository.d04f43961c819a58) | **The single most important invariant in the codebase.** One process, one data root, resolved once. What derives from it, how it propagates to child processes and packaged builds, the five sanctioned exceptions, the four known escape patterns, and — at length — what actually breaks when it is violated. |
...
| If you want to… | Read |
| --- | --- |
| Understand the product in one sitting | [overview.md](app-doc://article/material-designer.repository.e5a5102f18de6b16) |
| Call, extend or debug the local API | [daemon.md](app-doc://article/material-designer.repository.d2a69001b7e2ab97), then [../api/README.md](app-doc://article/material-designer.repository.ac0e4a04bf399f08) |
| Work out why the interface cannot reach the daemon | [web-runtime.md](app-doc://article/material-designer.repository.1c2b5224cec132bc) — there are three transports and only one applies |
| Change the window, its chrome, or anything privileged the renderer can reach | [desktop-shell.md](app-doc://article/material-designer.repository.bdd2e1ad8ecb5375) |
| Understand a shipped build, an install path, or an uninstall | [packaged-runtime.md](app-doc://article/material-designer.repository.224f711bf1d0aba7) |
| Add a path that writes anything to disk | [data-directory.md](app-doc://article/material-designer.repository.d04f43961c819a58) — **before** writing the code, not after |
...
[../standards/material-design-3.md](app-doc://article/material-designer.repository.e990d57fb20c3354).

## Suggested reading

- [data-directory.md](app-doc://article/material-designer.repository.d04f43961c819a58) — the invariant every one of these processes has to respect, and the one whose violation produces the least legible failure in the product
- [../porting/verification.md](app-doc://article/material-designer.repository.f5f04f6f9306bda7) — why the tree these processes are built from can be trusted to be what it claims
- ../build/from-source.md — getting these processes running locally, and the prerequisites that catch people out
- [../release/release-pipeline.md](app-doc://article/material-designer.repository.2ff912fbf46a71c0) — how the packaged form of all of this is produced and what proves it works
