...
| --- | --- |
| The HTTP surface | 38 route files, 351 handlers, 304 distinct path patterns. Enumerated in [../api/README.md](app-doc://article/material-designer.repository.ac0e4a04bf399f08), not repeated here. |
...
1. **Resolve the data root.** `OD_DATA_DIR` is read once and becomes
   ...
   has [its own page](app-doc://article/material-designer.repository.d04f43961c819a58).
...
Root options and the subcommand surface — flags, their defaults, and the dispatch map
...
| Variable | Default | Effect |
| --- | --- | --- |
| `OD_DATA_DIR` | `<project root>/.od` | The daemon data root. See [data-directory.md](app-doc://article/material-designer.repository.d04f43961c819a58). |
...
| Export to PDF, image or slides fails | Those paths rasterise through the desktop shell's browser engine | Not available from a bare daemon. See [desktop-shell.md](app-doc://article/material-designer.repository.bdd2e1ad8ecb5375). |
| A route with a bare `*` never matches | Express 5 requires **named** wildcards (`*splat`) | Rename the wildcard. |
| Data written by a feature is invisible to the rest of the app | A path that did not derive from `RUNTIME_DATA_DIR` | See [data-directory.md](app-doc://article/material-designer.repository.d04f43961c819a58) — this is the failure that page exists to prevent. |
...
from outside the process — see [../release/packaged-smoke-test.md](app-doc://article/material-designer.repository.3e55d384cd2c0acf).
...
inventory in [../api/README.md](app-doc://article/material-designer.repository.ac0e4a04bf399f08) is a transcription of source,
...
- [data-directory.md](app-doc://article/material-designer.repository.d04f43961c819a58) — the invariant every daemon path obeys, and what breaks when it does not
- [../api/README.md](app-doc://article/material-designer.repository.ac0e4a04bf399f08) — the full route inventory, grouped by the file that owns it
- [web-runtime.md](app-doc://article/material-designer.repository.1c2b5224cec132bc) — the client that consumes this API, and the three ways it reaches it
- [packaged-runtime.md](app-doc://article/material-designer.repository.224f711bf1d0aba7) — how the daemon is started in a shipped build
- ../build/from-source.md — getting a daemon running locally to test against
