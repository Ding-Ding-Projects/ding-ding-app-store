...
*Image omitted from the offline bundle: License.*
*Image omitted from the offline bundle: Platform.*
*Image omitted from the offline bundle: Electron.*
*Image omitted from the offline bundle: Node.*
*Image omitted from the offline bundle: pnpm.*
*Image omitted from the offline bundle: Design.*
*Image omitted from the offline bundle: Upstream.*

*Image omitted from the offline bundle: Material Designer running on Windows — the Material Design 3 home screen, showing the custom title bar, the prompt surface and the template rail.*

Not a mockup. This is the packaged application, captured by the smoke test that
installs the built installer, launches it and uninstalls it — built from commit
90e52d3. The capture path is the project's own and the image is committed
unedited. The navigation rail on the left and the 28px status bar along the bottom are
new: until 90e52d3 the rail was rendered into a zero-width track, so a
fresh install showed no rail at all.

*Image omitted from the offline bundle: The same screen in bilingual mode at a 900px window — Chinese and English paired on every label, with the status bar's density readout running off the right edge.*

Bilingual mode at the narrowest supported window, after the fix. The previous image
here showed the status bar's density readout running off the right edge and the Design
control truncated to Design · … — the segments were flex containers with
text-overflow: ellipsis, which does nothing to an anonymous flex item, so
...
where clipping appears first; that is why the capture set covers it.
...
More captures — the command palette, the settings dialog, the onboarding rename before and after, and the header search bar
...
*Image omitted from the offline bundle: The command palette open over the home screen, with Everything / Commands / Settings / Go to / Files and tabs scope chips and a grouped result list.*
...
*Image omitted from the offline bundle: The settings dialog with a horizontal tab strip — Execution mode, Instructions / Rules, Memory, Media providers — an overflow button reading 13, and a Search settings field with a regex toggle.*
...
*Image omitted from the offline bundle: The onboarding screen as it was, reading "Sign in to Open Design" with a footer reading copyright 2026 Open Design, inside a window titled Material Designer.*
...
*Image omitted from the offline bundle: The same onboarding screen after the rename, reading "Sign in to Open Design Cloud" — the heading gains the word Cloud — with the unchanged copyright 2026 Open Design footer, the Continue (signed in) button, and a tab truncated to "Welcome t...".*
...
*Image omitted from the offline bundle: The home screen at 200% UI scale after the fix — the heading wraps onto two lines, the navigation rail and status bar are both present, and there is no horizontal scrollbar.*
...
*Image omitted from the offline bundle: The home screen from release v0.16.1-r64.1 — an icon-only navigation rail on the left, a header search bar reading "Search projects, plugins, design systems…" with a .* regex chip and a Ctrl K chip beside it, the Material Designer wordmark over the heading "What will you design with your agent today?", a prompt composer, and a grid of template cards.*
...
The application source in `design/` is a byte-for-byte port of the Apache-2.0
...
`mockups/` — a minimal rebrand to *Material Designer*, and bringing the product
up to [the standards this project holds itself to](#standards). Every change to the ported
tree is declared in `MODIFICATIONS.md`, which is not a courtesy notice
...
files of upstream Open Design v0.16.1. `scripts/verify-port.sh`
...
> [!WARNING]
> **Compare those captures to the mockup before believing any claim about
...
Repository layout — what each top-level directory holds
...
Building from source — exact commands, toolchain versions and Windows prerequisites
...
written up under `docs/troubleshooting/`.
...
Verifying the port — running the verifier and reading its counters

The claim "`design/` is upstream, unmodified" is worth nothing unless a reader can check it
in one command. That is what `scripts/verify-port.sh` is for. It
...
Privacy and network defaults — telemetry, binding, and what this build does and does not send
...
Standards — the 16 requirements this project holds itself to, and which the roadmap burns down
...
The Material Design 3 mockup in `mockups/open-design-m3/` already
...
Provenance and licence — upstream, the pinned commit, Apache-2.0 and trademarks
...
**Licence.** Apache License 2.0. The full text ships at `design/LICENSE`
and applies to everything under `design/`. Work added by this repository is offered under the
same licence.

**The section 4(b) notice.** `MODIFICATIONS.md` is the prominent notice
...
**The submodule.** `vendor/open-design` stays checked in as a pinned
...
Feature documentation lives in `docs/` — one Markdown file per feature under a
...
[`docs/README.md`](app-doc://article/material-designer.repository.0b5ca119d2be595a).

Alongside it in the repository root:

- `ROADMAP.md` — what is planned, in what order, and what is explicitly not.
- `HANDOFF.md` — current state, verification evidence, and what the next
  session needs to know.
- `AGENTS.md` — the rules an agent working in this repository must follow.
- `MODIFICATIONS.md` — the licence notice and the enforced allowlist.

- `CHANGELOG.md` — every notable change, each linking the commit that made
...
editor and the narrator. `docs/standards/` records the honest position
...
`design/LICENSE` and `MODIFICATIONS.md`.
