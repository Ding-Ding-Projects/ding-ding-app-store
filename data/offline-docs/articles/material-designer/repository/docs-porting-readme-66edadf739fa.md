...
| File | What it covers |
| --- | --- |
| [verbatim-import.md](app-doc://article/material-designer.repository.c0c56c3fa44ddb9c) | How the copy was made: raw blob extraction with filters disabled, why a working-tree copy would not have worked, restoring the executable bit, and the two files that had to be force-added past upstream's own ignore rules. |
| [verification.md](app-doc://article/material-designer.repository.f5f04f6f9306bda7) | `scripts/verify-port.sh` in full: both independent checks, every counter it reports, the `MODIFICATIONS.md` allowlist contract, the self-test results, the line-ending trap that will break it in continuous integration, and how to run it. |
...
[verification.md § Reading a run](app-doc://article/material-designer.repository.f5f04f6f9306bda7#reading-a-run) — and the
...
- [../architecture/overview.md](app-doc://article/material-designer.repository.e5a5102f18de6b16) — what the imported tree actually is, once you trust that it is intact
- [../troubleshooting/line-endings.md](app-doc://article/material-designer.repository.a28170350a50b2c3) — the failure this category produces most often: thousands of differences against a tree nobody touched
- [../standards/README.md](app-doc://article/material-designer.repository.aa26f74331252403) — what is being changed in the verbatim copy, and the honest status of each requirement
- ../build/ci.md — where the verifier runs in continuous integration, and why the gate checks out without the submodule
