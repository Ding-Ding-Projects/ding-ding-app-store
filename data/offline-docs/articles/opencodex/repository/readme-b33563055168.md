...
```
      ...
      *Image or external asset omitted from the offline bundle.*
      Claude Code, running any model.The picker is stock Claude Code. The brain behind it isn't.
    
    
      *Image or external asset omitted from the offline bundle.*
      Codex, running any model.Pick a provider and go — same workflow, different brain.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Claude Desktop, dispatching subagents.Opus answers, and hands work to another vendor's model.
    
    
      *Image or external asset omitted from the offline bundle.*
      Grok Build, delegating.Same proxy, a fourth client.
...
not, rather than a button that fails when you press it.
  ...
  *Image or external asset omitted from the offline bundle.*
  Mobile remote. Chat, live sessions and proxy control, sized for a thumb.



  *Image or external asset omitted from the offline bundle.*
  Embedded terminal. No console window appears. The line you sent is recorded beside the output, so the log reads as a conversation.
...
Routing — what Codex is allowed to see


  
    
      *Image or external asset omitted from the offline bundle.*
      Providers. Grouped by state, with Edit JSON when the form is not enough.
    
    
      *Image or external asset omitted from the offline bundle.*
      Models. Toggle what reaches Codex's picker. Hidden models stay callable by exact id.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Combos. Virtual models that fail over, or spread load by weighted round-robin.
    
    
      *Image or external asset omitted from the offline bundle.*
      Sub-agents. spawn_agent only advertises five models — you pick which five.
...
Accounts, cost and what actually happened


  
    
      *Image or external asset omitted from the offline bundle.*
      Codex Auth. The pool, its quota windows, and which account routes next.
    
    
      *Image or external asset omitted from the offline bundle.*
      Usage. Local token accounting. Missing usage is shown as missing, never as zero.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Logs &amp; debug. Every request, newest first, filterable by client.
    
    
      *Image or external asset omitted from the offline bundle.*
      Storage. Cleanup previews before it deletes, and quarantines before that.
...
The clients — Claude Code, Grok, and anything that speaks HTTP


  
    
      *Image or external asset omitted from the offline bundle.*
      Claude. GPT and Gemini inside Claude Code — with or without a Claude account.
    
    
      *Image or external asset omitted from the offline bundle.*
      Grok. What opencodex has registered in your Grok config.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      API. Bearer keys, plus worked Chat Completions / Responses / Messages examples.
    
    
      *Image or external asset omitted from the offline bundle.*
      Startup. Stops "Codex forgot the proxy after a reboot" becoming a reconnect loop.
...
Making it yours — appearance, language, regex, settings


  
    
      *Image or external asset omitted from the offline bundle.*
      Appearance. Theme, density, seed colour and type — applied live, not on restart.
    
    
      *Image or external asset omitted from the offline bundle.*
      Language &amp; voice. Language mode, funny level, narrator, and the dim sum surprise.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Regex builder. Feeds every search bar with a .* toggle. Evaluated locally.
    
    
      *Image or external asset omitted from the offline bundle.*
      Settings. Every adjustable value in one place — and every change recorded.
...
Undo, reach and the paper trail


  
    
      *Image or external asset omitted from the offline bundle.*
      Version history. Append-only — restoring writes a new revision, so undo is undoable.
    
    
      *Image or external asset omitted from the offline bundle.*
      Remote access. Reach the proxy from other devices; export the whole state.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Changelog. Every release, date-filtered and searchable, exportable as Markdown.
    
    
      *Image or external asset omitted from the offline bundle.*
      Notifications. Nothing that toasted past you is lost — it is all still here.
...
Dialogs, menus and the tab strip


  
    
      *Image or external asset omitted from the offline bundle.*
      First run. Language first, four steps, and every one of them changeable afterwards.
    
    
      No admin-token prompt.Management routes stay reachable without a browser credential dialog.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Confirm. Blocking, because this one really does write every key to a file in plaintext.
    
    
      *Image or external asset omitted from the offline bundle.*
      Tab menu. Right-click stays ordinary tab management. Appearance is one entry further down.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Tab appearance. Anchored to the tab it edits, non-modal, and resettable one property at a time.
    
    
      *Image or external asset omitted from the offline bundle.*
      New tab. A filterable page list — the strip behaves like a browser, so a second tab is not a hunt.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Right-click, anywhere. The pointer sat inside two editable surfaces, so it offers both — nearest first.
    
    
      *Image or external asset omitted from the offline bundle.*
      Appearance editor. The same panel a tab gets, on a plain piece of page content.
...
The parts that appear on their own


  
    
      *Image or external asset omitted from the offline bundle.*
      Toasts. Corner-anchored and non-blocking. Errors stay until dismissed; nothing halts the app.
    
    
      *Image or external asset omitted from the offline bundle.*
      Notification centre. The newest few under the bell; the full history is one click on.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Cost meter. Lifetime by default. The chip changes the window without leaving the page.
    
    
      *Image or external asset omitted from the offline bundle.*
      Account switcher. Which account routes next, changed from the app bar rather than a settings page.
    
  
  
    
      *Image or external asset omitted from the offline bundle.*
      Anchored regex builder. It belongs to the field you are typing in, not to a page you navigate to.
    
    
      *Image or external asset omitted from the offline bundle.*
      Dim sum. A 1-in-100 launch treat. Never on a first run, never after an update, and there is a switch.
...
Account pooling — for every OAuth provider
...
adding another provider is optional.

`ocx init` is also optional. On a fresh setup, pressing Enter at its provider prompt
accepts provider **1**, the same OpenAI ChatGPT-login route with no key.

| Credential name | What it means |
| --- | --- |
| **ChatGPT/Codex login** | The default local route uses the account session from `codex login` or the Codex app. It is not an API key. |
| **Provider API key or account login** | Needed only for an additional upstream provider you choose to add. |
| **OpenCodex admission key** | Protects `/v1/*` when the proxy is exposed beyond `localhost`; it is generated by `ocx host enable --new-key --yes` and is not a provider billing key. |

The message **`opencodex API key required`** means a client reached a non-loopback listener without
that OpenCodex admission key. It does not mean the default ChatGPT route needs a provider API key.
See the [Quickstart](https://opencodex.me/getting-started/quickstart/)
for the local and LAN flows.


"bundled Bun runtime is missing" / npm blocked Bun's install scripts?
...
In the repository: `ROADMAP.md` for what is done and what is missing,
`HANDOFF.md` for work in flight, and `AGENTS.md` for the conventions
this codebase is built to. Maintainer source-of-truth notes live under `structure/`,
historical investigations under `docs/`, and contributor setup in
`CONTRIBUTING.md`.

Security reporting guidance lives in `SECURITY.md`. Report undisclosed
...
600,804 lines across 3,056 tracked files — the breakdown, and what is excluded
...
On Windows, `run.bat` in the repository root does all of that from one file — double-click
...
AGENTS.md. Sanitized of anything machine-, host- or account-specific.
