# *Image or external asset omitted from the offline bundle.* KeePassXC — Material
...
docs/topics/KeyboardShortcuts.adoc for shortcuts.
...
`vcpkg.json` and resolved by vcpkg.
...
Longer instructions, including the test invocations, live in INSTALL.md.
...
HANDOFF.md carries the full picture, including known defects and two things that
...
Contributors adhere to the project's Code of Conduct. See
CONTRIBUTING for the upstream workflow.
...
carries the same licence. Third-party file licensing is detailed in COPYING.
