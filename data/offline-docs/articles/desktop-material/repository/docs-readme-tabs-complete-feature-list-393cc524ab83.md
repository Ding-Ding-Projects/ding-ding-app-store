...
| **Opt-in post-clone Actions runner** — after one private GitHub clone, explicitly trust the workflow authors and create a repository-scoped Windows or dedicated WSL/Linux runner; public, batch, and background clone paths stay excluded.**自選 clone 後 Actions runner** — clone 一個 private GitHub repo 之後，明確確認信得過 workflow 作者，先至起 repository-scoped Windows 或專用 WSL/Linux runner；公開、批量同背景 clone 一律唔會整。 | **Added** | [Post-clone runner provisioning](app-doc://article/desktop-material.repository.f6c1064f42f95384) |
...
| **Self-hosted GitHub Actions runner manager** — set up and control a repository-scoped Windows runner only after private-repository, private-fork policy, immutable workflow, and pending-job audits pass. Exact-account tokens stay out of command-line arguments, interrupted operations retain recoverable lifecycle state, and WSL management remains disabled until process-group cancellation can be proved.**自架 GitHub Actions runner 管理器** — 私人倉庫、私人 fork 政策、不可變 workflow 同待處理 job 審核全部過關，先至可以設定同控制指定倉庫嘅 Windows runner。精確帳戶 token 唔會放入 command-line 參數；中斷操作會保留可復原生命週期狀態；WSL 管理要證明到成組 process 可以取消先會開放。 | **Added** | [Self-hosted runner manager](app-doc://article/desktop-material.repository.bf51e6cff280262c) |
