...
Date: 2026-07-20
Scope: post-remediation source and verification-harness review
...
| Fixed | Responsive registration | The catalog contains 86 rows: 85 product surfaces plus one clone-recovery row. Cheap LFS, Global ignore, account switcher, workflow manager, workflow catalog, and workflow dispatch are registered. | Fresh 86-row ledgers for every required viewport/theme/language combination with zero required-row failures. |
