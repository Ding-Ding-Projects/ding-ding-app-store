...
*Image omitted from the offline bundle: underlined access keys.*
...
File -> New Branch item a user should be able to press ALT+F
followed by B.
...
*Image omitted from the offline bundle: focused menu item.*
...
user should be able to expand the menu by pressing Enter or ↓.

Using the ← and → keys should move focus to the next
adjacent menu item in that direction, looping around if necessary.

### Expanded menus

An expanded menu respond to ↑ and ↓, moving the selection
...
→ should open a submenu if the currently selected item has one and
move selection (and focus) to the first item in that submenu. If the current
item does not have a submenu → should have no effect.
Similarly ← should close the currently open submenu and move focus
