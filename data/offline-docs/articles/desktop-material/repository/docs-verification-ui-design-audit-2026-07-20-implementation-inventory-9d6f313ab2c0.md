...
Date: 2026-07-20
Worktree: `desktop-material-ui-audit-20260720`
Scope: static implementation inventory for comparison with the supplied `Material Design UI Recreation (1).zip`
...
| Responsive catalog rows | 86 | 66 grouped rows plus 20 nested rows in `.codex/verification/responsive_surface_catalog.json`: 85 product surfaces and one deterministic paused/interrupted clone-recovery surface. |
...
The catalog is now statically current at 86 logical rows: 66 grouped surfaces and 20 nested surfaces, comprising 85 product rows plus one deterministic paused/interrupted clone-recovery row. The grouped set now includes Cheap LFS, and the nested set includes Preferences → Git → Global ignore, account switcher, workflow manager, workflow catalog, and workflow dispatch. The prior 79-row 2026-07-17 evidence remains historical and must not be reused as a post-fix runtime receipt.
...
| Responsive catalog and driver contracts | The exact 86-row topology includes Cheap LFS, Global ignore, and the four direct design surfaces; schema-v2 appearance, provider, mutation, and font gates are encoded in the verifier. | A post-fix 86-row runtime ledger across the required theme/language/viewports. |
...
| Existing responsive matrix | The 2026-07-17 ledger is historical evidence only. | Current topology and post-fix appearance/provider/font behavior; it predates the 86-row catalog. |
