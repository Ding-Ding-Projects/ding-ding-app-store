...
`AppStore` (see the Developer Guide) so agent-initiated actions obey the same
...
Where the app already gates an action (for example the Automation preconditions), the
