...
The [Guided Feature Gallery](app-doc://article/desktop-material.wiki.9f2c0c89337f30db) declares the canonical 86-scene
...
### If Check for Updates reports HTTP 404

The Windows updater reads the Squirrel `RELEASES` manifest from the published
`releases/latest/download/` asset path. A Release that contains only the
Linux/TUI payload cannot serve that manifest, so it must never become the
Windows `Latest` Release. The promotion helper now requires every `Latest`
candidate to carry both `RELEASES` and a full `*-full.nupkg` package; a newer
Linux/TUI-only Release therefore cannot replace a valid Windows feed. See the
promotion implementation
and its regression guard in commit
[`a4ce485037`](https://github.com/Ding-Ding-Projects/desktop-material/commit/a4ce485037138f24d7534452a861a1fb7749beeb).

The live alias was repaired on 2026-08-05 to the Windows-capable Release
[`v3.6.3-beta3-zadwftypqg`](https://github.com/Ding-Ding-Projects/desktop-material/releases/tag/v3.6.3-beta3-zadwftypqg),
and the exact `releases/latest/download/RELEASES` URL returned HTTP 200 during
verification. If a packaged build still shows the old 404, close and reopen
the About surface after the feed repair; the next check uses the corrected
alias. The focused regression suite passed **29/29**.

廣東話：Windows updater 要搵 Squirrel 嘅 `RELEASES` 檔。淨係 Linux/TUI 資產嘅
Release 冇呢扇門，所以 promotion 而家會先驗齊 Windows 資產，唔畀佢霸住
`Latest`。2026-08-05 修好 live alias，實際 feed 已經由 404 變返 200。
...
2. The list shows every repository available to the active account. Use the **search bar** — with
   filter chips, regex mode, and the regex builder (see the [Regex Guide](app-doc://article/desktop-material.wiki.5ca9bc45a79f3aeb)) — to narrow
   it down.
...
see [Automation](app-doc://article/desktop-material.wiki.81fc87a3020bb04a).
...
- The repository's **Super Express Release** emergency lane runs the complete
  ...
  the comprehensive gates. A release
...
repository is skipped rather than overwritten. See [Automation](app-doc://article/desktop-material.wiki.81fc87a3020bb04a) for the complete guard
table.
...
- Open the wider **Repository settings → Submodules** surface and choose **Add submodule…** to browse GitHub.com,
  ...
  **Subtrees** tab embeds the full add, pull, push, and split manager. The same managed list opens
  ...
  New to submodules? The beginner-friendly [Submodules](app-doc://article/desktop-material.wiki.72b7b180f6dd8a34) page walks the whole workflow
...
returns provider credentials. See [Agent API](app-doc://article/desktop-material.wiki.45d5415c08e37aab) for command and security details.

*Image omitted from the offline bundle: Agent access connection and token controls.*

---

**Next:** [Automation](app-doc://article/desktop-material.wiki.81fc87a3020bb04a) · [Submodules](app-doc://article/desktop-material.wiki.72b7b180f6dd8a34) · [Regex Guide](app-doc://article/desktop-material.wiki.5ca9bc45a79f3aeb) · [Developer Guide](app-doc://article/desktop-material.wiki.08e927aab695accc)
