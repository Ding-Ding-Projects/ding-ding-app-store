...
anchored full regex builder. [Read the UI guide](app-doc://article/jdownloader-material.repository.ea53de5a3bcbba32), explore the
[design system](app-doc://article/jdownloader-material.repository.b9356668f945997c), or try the
...
| Light | Dark |
| --- | --- |
| *Image omitted from the offline bundle: Downloads in the light theme.* | *Image omitted from the offline bundle: Downloads in the dark theme.* |

| Status feedback | Selected item, light | Selected item, dark |
| --- | --- | --- |
| *Image omitted from the offline bundle: Downloads with fixed activity feedback.* | *Image omitted from the offline bundle: Queued-download properties in the light theme.* | *Image omitted from the offline bundle: Queued-download properties in the dark theme.* |

| Hong Kong Cantonese | Bilingual English / Hong Kong Cantonese |
| --- | --- |
| *Image omitted from the offline bundle: Downloads in playful Hong Kong Cantonese.* | *Image omitted from the offline bundle: Downloads in bilingual English and Hong Kong Cantonese.* |

### LinkGrabber

| Light | Dark | Hong Kong Cantonese |
| --- | --- | --- |
| *Image omitted from the offline bundle: LinkGrabber in the light theme.* | *Image omitted from the offline bundle: LinkGrabber in the dark theme.* | *Image omitted from the offline bundle: LinkGrabber in playful Hong Kong Cantonese.* |

### History

| Light | Dark | Bilingual English / Hong Kong Cantonese |
| --- | --- | --- |
| *Image omitted from the offline bundle: History in the light theme.* | *Image omitted from the offline bundle: History in the dark theme.* | *Image omitted from the offline bundle: History in bilingual English and Hong Kong Cantonese.* |
...
| --- | --- | --- |
| *Image omitted from the offline bundle: Searchable changelog with date filters, copy, and export.* | *Image omitted from the offline bundle: Strict-loopback installed-JDownloader plugins bridge.* | *Image omitted from the offline bundle: Searchable notification history in bilingual mode.* |

### Responsive layout and startup delight

| Bilingual compact width | Non-blocking dim-sum surprise |
| --- | --- |
| *Image omitted from the offline bundle: Downloads at 880 by 560 pixels in bilingual mode.* | *Image omitted from the offline bundle: Non-blocking shrimp-dumpling startup card over Downloads.* |
...
The maintained [UI smoke handoff](app-doc://article/jdownloader-material.repository.d80ab157289480e9) records the route, accessibility, compact-layout,
...
The [categorized documentation index](app-doc://article/jdownloader-material.repository.0b5ca119d2be595a) describes the implemented desktop behavior.
The factual roadmap and current handoff keep local implementation evidence
