...
These commands validate the WimForge Bridge recipe/contract and stage its OEM bundle. They do not invoke guessed legacy switches such as `--apply-recipe` or `--apply-tweak`. The audited legacy WinForge runtime itself supports only `--page <alias>` as a relevant public capability. See [WinForge Bridge](app-doc://article/wimforge.wiki.756d93d5550cf500).
...
The full command/field reference lives in `docs/cli.md`. Run `.\WimForgeCli.exe help` for the exact command set compiled into the current binary.

---

[← Project Bundles](app-doc://article/wimforge.wiki.6fb52219fd512f87) · [WinForge Bridge →](app-doc://article/wimforge.wiki.756d93d5550cf500)
