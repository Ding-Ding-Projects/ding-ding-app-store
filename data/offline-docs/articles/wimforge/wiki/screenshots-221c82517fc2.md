...
*Image omitted from the offline bundle: WimForge Project Start showing bilingual create, open, import, and recent-project actions.*

呢個係 app 開啟後嘅第一個畫面：可以建立新工程、開啟現有資料夾、匯入 `.json` / `.wimforge`，或由最近清單繼續。

### Overview

*Image omitted from the offline bundle: WimForge Overview showing project metrics, the five-step build flow, safety rails, and current-job status / WimForge 總覽顯示工程指標、五步建置流程、安全護欄同目前工序狀態.*

### Source and editions

*Image omitted from the offline bundle: Source and editions showing neutral ISO and working-image paths, clone-before-editing, edition selection, mount path, and output format.*

### Customize

*Image omitted from the offline bundle: Customize showing the update and language-package queue, navigation across all configuration categories, and a neutral demo payload.*

### Group Policy Studio

*Image omitted from the offline bundle: Group Policy Studio showing the installed ADMX catalog, a selected Delivery Optimization policy, three-state draft controls, a schema-generated numeric editor, registry target, and Git-backed commit action.*

### Unattended Studio

*Image omitted from the offline bundle: Unattended Studio showing computer-name behavior, Microsoft-published installation keys, the generic answer-file editor, and setup-pass values.*

### Package Studio

*Image omitted from the offline bundle: Package Studio showing the Full AI Development profile, provider-backed software cards, selection count, and staging controls.*

### WinForge Bridge

*Image omitted from the offline bundle: WinForge Bridge showing typed recipe actions, runtime contract detection, portable recipe controls, and verified ISO staging.*

### Virtual Machine Lab

*Image omitted from the offline bundle: Virtual Machine Lab in its safe empty state, showing unavailable provider discovery, inventory filters, workflow tabs, and no selected machine.*

### Review and run

*Image omitted from the offline bundle: Review and run showing the reviewed operation list, deterministic verification steps, concurrency control, checkpoint setting, and explicit run button.*

### History and recovery

*Image omitted from the offline bundle: History Time Machine showing append-only actions, branch and bookmark controls, guarded undo and restore actions, and the A/B comparison pane.*

### Settings

*Image omitted from the offline bundle: Settings showing language, Material theme, interface density, motion, concurrency, scratch-space reserve, and failsafe controls.*

### Embedded terminal

*Image omitted from the offline bundle: Embedded Terminal ready state showing a trusted administrator shell selector, neutral working directory, empty bounded ConPTY viewport, command input, and inactive stop controls.*
...
*Image omitted from the offline bundle: WimForge documentation home at the desktop viewport.*

*Image omitted from the offline bundle: WimForge documentation home at the mobile viewport.*
...
See [Safety and Recovery](app-doc://article/wimforge.wiki.a57c84860be90dc5), [Troubleshooting](app-doc://article/wimforge.wiki.0278323fb6071d70),
and [Contributing](app-doc://article/wimforge.wiki.1246fcebc419eba2).

---

[← Application Tour](app-doc://article/wimforge.wiki.f1ac27ad46b00247) · [Troubleshooting →](app-doc://article/wimforge.wiki.0278323fb6071d70)
