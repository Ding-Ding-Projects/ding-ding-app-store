...
Human Nudge is a final command shaper in `DefenderStateStore`. `AcDefenderService` calls it after Natural Walkback and Touch Signature calculate the candidate setpoint, and before Repeat Quiet checks for duplicate commands. It rewrites only safe, non-bypass commands into normal thermostat-looking step values and clears when direct comfort correction is needed.
