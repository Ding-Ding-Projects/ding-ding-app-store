...
detailed Settings history filters and exports article for failure,
...
Home Assistant state, or a defender guard. See the detailed Appearance editor
...
Suggested next steps: Website Tour, Deployment, and
Architecture.
...
- Nudge size C: caps softer non-warm corrections. Warm-room cooling starts `WarmRoomApproachCelsius` below current room temperature (0.5 C by default).
...
Settings → Electricity budget (see Energy & Costs for the full behaviour):
...
- **Electricity cost tracking** rates and all-in bill factors (`Electricity*`) and the
  **AC-only cost estimate** load (`AcCostEstimateEnabled`, `AcEstimatedAmps`,
  `AcEstimatedVolts`) — see Energy & Costs.
