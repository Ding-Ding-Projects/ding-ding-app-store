...
description: "A searchable plain-language guide to every AC Defender algorithm."
---

# Every Guard, Explained Simply

AC Defender is for the person who is tired of waking up hot because the thermostat keeps drifting warmer. The product does not pretend to own a fake thermostat. It watches the real Home Assistant climate entity, figures out whether a change came from a person, a phone, a schedule, weather, or energy pressure, then gets back to the chosen temperature as safely and naturally as possible.


  
    Plain-language map
    Find the guard that explains the behavior.
    Search by symptom, input, or setting. Open a card for the full article with exact watches, decision logic, output, and safety notes.
  ...
  *Image or external asset omitted from the offline bundle.*
  ...
  Search guards
  Clear
  50 guards shownNo guard matches that search.
...
**The room comes back to the target you chose.** Timing guards may wait, stealth guards may soften, energy guards may pace safe cooling, and human-courtesy guards may stand down inside the safety band. Direct comfort and real safety still win when the room needs cooling.

## The four families

| Family | What it means |
| --- | --- |
| **Core Cooling** | The always-on spine that keeps the AC in cooling mode and gets warm rooms moving back toward the chosen target. |
| **Wall-Touch Response** | The courtesy and stealth layer for real thermostat touches from people in the house. |
| **Sensor Timing** | Timing that lines corrections up with real Home Assistant readings, HVAC action, weather, and usage telemetry. |
| **Safety, Energy, and System** | Safety protocols, remote-change handling, energy policy, schedules, emergency controls, and owner-enforcement rules. |


Want the exact rule text? Use Defender Logic. Want a single page for one guard? Use Algorithms.

## Failure modes

If **Every Guard, Explained Simply** cannot obtain one of its required real inputs, it reports a blocked, held, or unavailable result and leaves the background worker's Home Assistant refresh running. It never fills a missing room reading, audit event, weather sample, usage value, or device state with a simulator value. If a real Home Assistant command is rejected, the user sees the service's actual error and the article's surface remains available for recovery.
