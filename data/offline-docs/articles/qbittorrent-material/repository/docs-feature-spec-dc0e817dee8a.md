...
1. `m_freeDiskSpaceLbl` (QLabel) — "Free space: N/A" → "Free space: ". Updated from `Session::freeDiskSpace()` and `freeDiskSpaceChecked(qint64)`. Visible per `Preferences::isStatusbarFreeDiskSpaceDisplayed()`.
...
1. **"Status"** — `StatusFilterWidget`. Checked state key `Preferences::getStatusFilterState/setStatusFilterState`. Fixed rows (label pattern " ()"), backed by `TorrentFilter` enum: **All, Downloading, Seeding, Completed, Running, Stopped, Active, Inactive, Stalled, Stalled Uploading, Stalled Downloading, Checking, Moving, Errored**.
...
- **Title** (`refreshWindowTitle`): base = `"qBittorrent " QBT_VERSION` + optional " — ". Paused → `"[PAUSED] %1"`. Speed-in-title → `"[D: %1, U: %2] %3"`.
...
2. **Results count label** — `QLabel resultsLbl`: "Results (showing %1 out of %2):" (filtered / total).
3. horizontal spacer
...
### `RSS::AutoDownloader` (singleton, ApplicationComponent) — `rss_autodownloader.h`
Settings:
