# Wildline Nation catalog record
...
This record describes the reviewed catalog entry `wildline-nation`. Its public source repository is [wildline-nation](https://github.com/Ding-Ding-Projects/wildline-nation). The current availability is **Installable through a reviewed adapter** and the declared package type is **squirrel**. The closed adapter identifier is `wildline-nation-squirrel`.

The current adapter state is: Reviewed Squirrel.Windows adapter. The adapter has a fixed, application-specific release contract; this article deliberately does not reproduce executable names, arguments, download URLs, or filesystem locations. Lifecycle status is **blocked until clean-Windows proof** (target wildline-nation-clean-windows); the catalog does not claim installation, launch, ownership, or uninstall evidence yet.

The icon is first-party reviewed from repository asset `site/wildline-card.svg`; if that asset is unavailable, the UI uses the declared generated-monogram fallback rather than a remote or guessed image.
