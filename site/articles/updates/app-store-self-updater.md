...
`tests/update-service.test.ts` covers full/delta parsing, newest-full selection, malformed metadata, conflicting duplicate rows, and immutable release-note links. The main-process and renderer TypeScript builds pass on this branch. A prior packaged run reached the real feed and truthfully reported HTTP 404; this lane does not claim a successful download, hash-mismatch event, restart, rollback on a clean Windows VM, or a newly published release asset, so status remains limited at the runtime/release boundary.
