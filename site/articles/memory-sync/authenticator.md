---
id: authenticator
title: Local authenticator registration and entries
titleYue: 本機驗證器登記同項目
category: memory-sync
status: limited
summary: Registers bounded TOTP metadata through local otpauth or Base32 input, confirms pairing, and keeps secrets behind the operating-system credential vault.
---
...
The source, algorithm, and digit fields use bounded keyboard-first pickers. Each picker owns its local plain-text query and optional regex state, opens an anchored full regex builder, reports an honest no-match state, and supports Arrow/Home/End navigation, Enter selection, Escape cancellation, and focus return to its trigger. Registration semantics are unchanged: source changes clear pending preview and entered secret/URI, and all three pickers are disabled while a pairing preview is active. The period remains a bounded numeric field because it is a free-form 1–3600 second value rather than an enumerated choice.
