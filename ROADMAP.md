...
- [ ] Prove a second MSI adapter on clean Windows: the typed dispatch lane admits `codex-material-msi` with direct SHA-256, empty initial target state, exact registry ownership, MSI product-code uninstall, and empty detected/persisted cleanup requirements; exact-commit cloud evidence is pending
