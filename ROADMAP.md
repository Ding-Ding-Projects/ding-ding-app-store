...
- [ ] Authenticator follow-up: QR image/clipboard/camera import, deliberate secret export, stable group records and group-level actions, protected secret history/restore, and localized prepare/confirm bridge-rejection notifications remain pending; the next-code peek and visible-range selection are shipped; picker filter-input keyboard handoff, localized builder copy, preview-lock rejection, result counts, and bounded overlay scrolling are shipped
