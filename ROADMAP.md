...
- [ ] Authenticator follow-up: local clipboard `otpauth://totp/` import is shipped with sender validation, School-mode denial, bounded UTF-8 input, and in-memory normalization; QR image/camera import, deliberate secret export, stable group records and group-level actions, and protected secret history/restore remain pending. The next-code peek, visible-range selection, and localized non-blocking prepare/confirm bridge-rejection notifications are shipped.
