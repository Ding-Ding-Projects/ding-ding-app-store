...
- [ ] Prove the first MSI adapter on clean Windows: the typed dispatch lane now admits only `keepassxc-msi` beyond the already verified portable/Squirrel targets and requires direct SHA-256, empty initial target state, exact registry ownership, MSI product-code uninstall, and empty detected/persisted cleanup state; exact-commit cloud evidence is pending
