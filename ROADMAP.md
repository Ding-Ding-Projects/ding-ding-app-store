...
- [x] Bounded tab/group and appearance-property UX locks with independent password/TOTP credentials, encrypted main-process vault storage, rate limiting, fail-closed metadata handling, sender-checked appearance IPC, and restore refusal while appearance locks are active; QR pairing, skew-aware OTP parameters, unlock durations, bulk/global scopes, and protected lock-history restore remain outside this slice
...
- [ ] Authenticator follow-up: local clipboard `otpauth://totp/` import and bounded local QR image-file import are shipped with sender validation, School-mode denial, bounded input, and in-memory normalization; camera scanning, deliberate secret export, stable group records and group-level actions, and protected secret history/restore remain pending. The next-code peek, visible-range selection, and localized non-blocking prepare/confirm bridge-rejection notifications are shipped.
