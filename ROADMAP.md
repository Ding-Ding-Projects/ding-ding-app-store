...
- [x] Local version browser: bounded Git snapshots with diff, labels, coordinated live reload, durable interrupted-restore recovery, and explicit before/after restore revisions for settings, installed records, workspace, appearance, schedules, run metadata, and external-editor preference; credential vaults and staged update paths remain excluded
...
- [ ] Authenticator follow-up: QR image/clipboard/camera import, deliberate secret export, stable group records and group-level actions, shift-range selection, protected secret history/restore, and localized prepare/confirm bridge-rejection notifications remain pending; the next-code peek is shipped; picker filter-input keyboard handoff, localized builder copy, preview-lock rejection, result counts, and bounded overlay scrolling are shipped
