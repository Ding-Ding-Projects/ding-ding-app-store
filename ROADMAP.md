...
- [ ] Prove the first NSIS adapter on clean Windows: the typed dispatch lane admits `material-email-nsis` with direct SHA-256, empty initial target state, exact registry ownership, reviewed-executable uninstall, and empty detected/persisted cleanup requirements; exact-commit cloud evidence is pending
