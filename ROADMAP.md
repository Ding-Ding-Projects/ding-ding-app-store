...
- [ ] Prove the first non-portable adapter on clean Windows: the typed dispatch-only lane now admits only `qbittorrent-material-squirrel` beyond the three portable targets and requires direct SHA-256, empty initial target state, exact new registry ownership, Squirrel uninstall, and empty detected/persisted cleanup state; an exact-commit `windows-2022` dispatch verdict is still pending
