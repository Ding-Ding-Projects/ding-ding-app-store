...
- [ ] Authenticator follow-up: QR image/clipboard/camera import, deliberate secret export, stable group records and group-level actions, and protected secret history/restore remain pending; the next-code peek, visible-range selection, and localized non-blocking prepare/confirm bridge-rejection notifications are shipped; picker filter-input keyboard handoff, localized builder copy, preview-lock rejection, result counts, and bounded overlay scrolling are shipped
