...
- [ ] Authenticator follow-up: QR image/clipboard/camera import, next-code peek, deliberate secret export, stable group records and group-level actions, shift-range selection, protected secret history/restore, and localized prepare/confirm bridge-rejection notifications remain pending; picker filter-input keyboard handoff, localized builder copy, preview-lock rejection, result counts, and bounded overlay scrolling are shipped
