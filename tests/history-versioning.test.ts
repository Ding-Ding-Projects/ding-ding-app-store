...
describe('local history/version browser contract', () => {
  ...
  it('records labels and restores as append-only commits with a before snapshot', async () => {
    ...
    expect(service).toContain('await this.snapshotUnlocked(restoreLabel, true)');
    expect(service).toContain('const restoreLabel = `restore: ${id} (${randomUUID()})`;');
    expect(service).toContain('content === null');
    const transaction = await read('src/main/history-restore-transaction.ts');
    expect(transaction).toContain("phase: 'applying'");
    expect(transaction).toContain("phase: 'recording'");
    expect(transaction).toContain('stagedSha256');
    expect(transaction).toContain('previousSha256');
