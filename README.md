...
- A top-level Authenticator tab provides bounded local URI/Base32 TOTP registration, in-process QR matrices, pairing confirmation, safeStorage-backed metadata entries, current codes, countdowns, regex search, metadata-only rename/reorder, per-entry label-only grouping, checkbox selection, destructive single/bulk delete, and redacted JSON/CSV/Markdown export with an optional Open in VS Code action. Its source, algorithm, digit, and export-format choices are keyboard-searchable bounded pickers with independent anchored regex builders; listbox keyboard navigation is complete once focus leaves the filter input, while filter-input handoff and localized prepare/confirm bridge-rejection notifications remain explicit follow-ups. QR image/camera import, next-code peek, deliberate secret export, stable group entities, shift-range selection, and protected authenticator history/restore remain explicitly deferred.
...
- Limited tab/group UX locks and a local Support Tickets desk are available from each target's context menu, Settings → Locks & Support, and Help. Locks are password-only, session-scoped speed bumps backed by Electron `safeStorage` when available; vault failure stays visible and fail-closed. Tickets never leave this device and only open the exact app-data folder for a user-led reset; per-property and OTP locks are not claimed.
