...
The common dispatch is implemented, and a manual [Windows cloud install-proof workflow](.github/workflows/install-adapter-proof.yml) now drives the three reviewed portable adapters through real install, exact rediscovery, and cleanup on `windows-2022`. Each remaining executable adapter still needs its own narrowly reviewed proof lane; the current [24-app adapter matrix](docs/features/installation/one-click-installation.md) records every missing release, archive, source-toolchain, dependency-bootstrap, disposable-runner, bounded OpenCode-repair, cancellation, and runtime-proof requirement without treating a guessed command as an adapter.
