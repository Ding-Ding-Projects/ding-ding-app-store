...
- A top-level Authenticator tab provides bounded local URI/Base32 TOTP registration, local clipboard and QR image-file import, in-process QR matrices, pairing confirmation, safeStorage-backed metadata entries, current/next-period codes, countdowns, regex search, metadata-only rename/reorder, stable UUID-backed groups with colour/order/collapse controls, a keyboard-searchable regex move picker, two-key/full-slider group deletion, visible-range selection, destructive single/bulk entry delete, and redacted JSON/CSV/Markdown export with an optional Open in VS Code action. Clipboard and QR-image input are sender-validated, School-mode gated, byte-bounded, normalized in memory, and never network-fetched or persisted outside the existing confirmation flow. Camera scanning, deliberate secret export, and protected authenticator history/restore remain explicitly deferred.
