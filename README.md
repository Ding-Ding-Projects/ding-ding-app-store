...
- A top-level Authenticator tab provides bounded local URI/Base32 TOTP registration, local clipboard import for one `otpauth://totp/` URI, local QR image-file import through the native picker and bundled decoder, in-process QR matrices, pairing confirmation, safeStorage-backed metadata entries, current and next-period codes, countdowns, regex search, metadata-only rename/reorder, per-entry label-only grouping, checkbox plus Shift-click/Shift+Space visible-range selection, destructive single/bulk delete, and redacted JSON/CSV/Markdown export with an optional Open in VS Code action. Clipboard and QR-image input are sender-validated, School-mode gated, byte-bounded, normalized in memory, and never network-fetched or persisted outside the existing confirmation flow. Camera scanning, deliberate secret export, stable group entities, and protected authenticator history/restore remain explicitly deferred.
- Activity and Local versions now have a separate protected-history credential gate. The verifier is encrypted with the operating-system credential vault, only a short-lived main-process session is unlocked, all history IPC routes are sender-checked and School-mode aware, and ordinary exports remain redacted. Authenticator metadata/ciphertext are still outside plaintext local-Git snapshots; vault-native authenticator restore remains deferred.
