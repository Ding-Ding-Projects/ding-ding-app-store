...
- A top-level Authenticator tab provides bounded local URI/Base32 TOTP registration, in-process QR matrices, pairing confirmation, safeStorage-backed metadata entries, current codes, countdowns, and regex search. Its source, algorithm, and digit choices are keyboard-searchable bounded pickers with independent anchored regex builders; QR image/camera import, next-code peek, rename/reorder/groups/bulk/secret export, and protected authenticator history remain explicitly deferred.
