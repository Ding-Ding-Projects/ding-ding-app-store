...
- A top-level Authenticator tab provides bounded local URI/Base32 TOTP registration, in-process QR matrices, pairing confirmation, safeStorage-backed metadata entries, current and next-period codes, countdowns, regex search, metadata-only rename/reorder, per-entry label-only grouping, checkbox plus Shift-click/Shift+Space visible-range selection, destructive single/bulk delete, and redacted JSON/CSV/Markdown export with an optional Open in VS Code action. Successful confirmation and metadata mutations also append localized redacted Activity events containing fixed action text plus opaque entry ID/count metadata only; recorder failure never changes the truthful mutation result. Its source, algorithm, digit, and export-format choices are keyboard-searchable bounded pickers with independent anchored regex builders; filter-input Arrow/Home/End/Enter/Escape handoff, localized builder copy, preview-lock rejection, result counts, viewport-bounded scrolling, in-flight prepare fencing with opaque lost-response cleanup, unknown-confirm-outcome refresh, and localized non-blocking notifications for prepare/confirm bridge, parser, and transport rejection are covered. QR image/camera import, deliberate secret export, stable group entities, and protected authenticator history/restore remain explicitly deferred.
