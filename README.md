...
- A top-level Authenticator tab provides bounded local URI/Base32 TOTP registration, in-process QR matrices, pairing confirmation, safeStorage-backed metadata entries, current and next-period codes, countdowns, regex search, metadata-only rename/reorder, per-entry label-only grouping, checkbox selection, destructive single/bulk delete, and redacted JSON/CSV/Markdown export with an optional Open in VS Code action. Its source, algorithm, digit, and export-format choices are keyboard-searchable bounded pickers with independent anchored regex builders; filter-input Arrow/Home/End/Enter/Escape handoff, localized builder copy, preview-lock rejection, result counts, and viewport-bounded scrolling are covered. QR image/camera import, deliberate secret export, stable group entities, shift-range selection, and protected authenticator history/restore remain explicitly deferred.
