# Material UniGetUI catalog record
...
This record describes the reviewed catalog entry `material-unigetui`. Its public source repository is [material-unigetui](https://github.com/Ding-Ding-Projects/material-unigetui). The current availability is **Installable through a reviewed adapter** and the declared package type is **squirrel**. The closed adapter identifier is `material-unigetui-squirrel`.

The current adapter state is: Reviewed Squirrel.Windows adapter. The adapter has a fixed, application-specific release contract; this article deliberately does not reproduce executable names, arguments, download URLs, or filesystem locations. Lifecycle status is **blocked until clean-Windows proof** (target material-unigetui-clean-windows); the catalog does not claim installation, launch, ownership, or uninstall evidence yet.

The icon is first-party reviewed from repository asset `build/icon.ico`; if that asset is unavailable, the UI uses the declared generated-monogram fallback rather than a remote or guessed image.

## Configuration

The catalog record is source-controlled. The renderer may request this application by its typed identifier and a user decision, but it cannot alter the repository, source manifest, package type, adapter, download, command, argument, or local destination. The source manifest marker is `app/package.json`; it is metadata only and is not a source-build recipe.
