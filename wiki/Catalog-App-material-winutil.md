# Material System Utility catalog record
...
This record describes the reviewed catalog entry `material-winutil`. Its public source repository is [material-winutil](https://github.com/Ding-Ding-Projects/material-winutil). The current availability is **Installable through a reviewed adapter** and the declared package type is **squirrel**. The closed adapter identifier is `material-winutil-squirrel`.

The current adapter state is: Reviewed Squirrel.Windows adapter. The adapter has a fixed, application-specific release contract; this article deliberately does not reproduce executable names, arguments, download URLs, or filesystem locations. Lifecycle status is **blocked until clean-Windows proof** (target material-winutil-clean-windows); the catalog does not claim installation, launch, ownership, or uninstall evidence yet.
