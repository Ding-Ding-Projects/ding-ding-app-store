...
The dispatch-only `.github/workflows/install-adapter-proof.yml` workflow currently targets only the reviewed `dim-sum-atlas` portable ZIP adapter on a pinned GitHub-hosted `windows-2022` cloud runner. It bootstraps the locked dependencies, builds the real main process and preload, launches `scripts/prove-install-adapter.mjs` through Electron with a disposable app-data root, records bounded progress plus before/after installed detection, and cleans up through the reviewed uninstall route. The proof manifest contains no registry paths, credentials, installer bytes, or source/OpenCode output. This first proof is green only when the portable install succeeds, the exact App Store-owned installed record is rediscovered, uninstall succeeds, and the record disappears; the harness rejects every other app ID until a separate app-specific proof boundary is reviewed. Manual dispatch is intentional because installing third-party software is a side effect, not a push-time check.
