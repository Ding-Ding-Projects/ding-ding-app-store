...
import { LIFECYCLE_PRODUCT_IDS, LIFECYCLE_PROOF_SCHEMA } from './lifecycle-proof-matrix.mjs';
...
try {
  ...
  for (const name of names) {
    ...
    if (parsed.schemaVersion !== LIFECYCLE_PROOF_SCHEMA || !Array.isArray(parsed.receipts)) throw new Error(`Receipt ${name} is not a lifecycle-proof.v2 aggregate.`);
