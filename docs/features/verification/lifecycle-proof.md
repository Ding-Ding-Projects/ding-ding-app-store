...
The current isolated lane supplies the harness contract, recipe receipt persistence, and attested-driver seam. The real disposable guest, source-build, process/window, install, uninstall, and absence evidence remain blocked unless a driver proves the integrated Windows Sandbox peer and each exact recipe vector. No runtime lifecycle success is claimed from source-level checks or from a driver that is not explicitly marked `windows-sandbox-attested`.
