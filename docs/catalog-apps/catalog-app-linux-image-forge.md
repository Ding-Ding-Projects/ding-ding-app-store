# Linux Image Forge catalog record
...
This record describes the reviewed catalog entry `linux-image-forge`. Its public source repository is [linux-image-forge](https://github.com/Ding-Ding-Projects/linux-image-forge). The current availability is **Installable through a reviewed adapter** and the declared package type is **squirrel**. The closed adapter identifier is `linux-image-forge-squirrel`.

The current adapter state is: Reviewed Squirrel.Windows adapter. The adapter has a fixed, application-specific release contract; this article deliberately does not reproduce executable names, arguments, download URLs, or filesystem locations. Lifecycle status is **blocked until clean-Windows proof** (target linux-image-forge-clean-windows); the catalog does not claim installation, launch, ownership, or uninstall evidence yet.

The icon is first-party reviewed from repository asset `assets/icon.ico`; if that asset is unavailable, the UI uses the declared generated-monogram fallback rather than a remote or guessed image.
